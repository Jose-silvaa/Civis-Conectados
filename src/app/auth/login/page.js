...
export default function Login(){
    ...
    const handleSubmit = async(e) =>{
      ...
      try {
        ...
        if (user) {
          ...
          setTimeout(() => {
            router.replace("/account/inbox"); // Redireciona para a página principal
          }, 3000);
    ...
    <main className="flex items-center justify-center min-h-screen bg-gray-100">
      <section className="w-full max-w-md p-8 space-y-6 bg-white rounded-lg shadow-lg">
        ...
        {loginSuccess && (
          <p className="text-green-500 text-center mt-4">Login realizado com sucesso! Redirecionando....</p>
        )}
      </section>
    </main>
    )
